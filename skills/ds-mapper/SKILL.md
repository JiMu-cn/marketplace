...
1. **扫描根目录** — 优先使用积木 `list_directory` / `glob` 获取顶层结构，不用 shell 的 `find` 或 `ls` 替代专用工具
2. **分析关键目录** — 使用 `read_file` 读取关键文件（package.json, Cargo.toml, pyproject.toml, go.mod 等）获取项目元信息
3. **递归扫描** — 使用 `glob` 深入扫描主要目录，理解各模块职责
