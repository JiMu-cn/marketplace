...
2. **Explore** - Map structure using Jimu tools: `glob` for file discovery, `search_file_content` for code search, and `read_file` for source inspection.
...
glob(pattern: "**/*.py", respect_git_ignore: true)

# Locate technical debt markers
search_file_content(pattern: "TODO|FIXME|HACK|XXX", include: "*.py")

# Discover configuration and environment usage
search_file_content(pattern: "os\\.environ|config\\[|settings\\.", include: "*.py")

# Map API route definitions (Flask/Django/Express examples)
search_file_content(pattern: "@app\\.route|@router\\.|router\\.get|router\\.post", include: "*.py")
```
...
|-------|-----------|-----------|
| Analysis Process | `references/analysis-process.md` | Starting exploration, `glob` / `search_file_content` patterns |
...
- Use `read_file`, `search_file_content`, and `glob` extensively to explore
