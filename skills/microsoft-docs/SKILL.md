...
name: microsoft-docs
description: |
  Query official Microsoft documentation to find concepts, tutorials, and code examples across Azure, .NET, Agent Framework, Aspire, VS Code, GitHub, and more. Use Jimu-available web/document retrieval tools by default; Microsoft Learn MCP, Context7, and Aspire MCP references are optional upstream integrations when configured.
author: GitHub Community
license: 
category: document
tags: Microsoft, 文档检索, Learn, Azure, .NET
source: https://github.com/github/awesome-copilot/tree/main/skills/microsoft-docs
---
...
## Default: Jimu-Available Microsoft Documentation Research

Research learn.microsoft.com with the documentation retrieval tools available in Jimu. If Microsoft Learn MCP is configured, the upstream tool names below can be used; otherwise use Jimu web search/fetch or other configured document tools.

| Optional upstream MCP tool | Purpose |
|------|---------|
...
If the Microsoft Learn MCP is configured, use `microsoft_docs_fetch` after search when complete tutorials or full config options are needed; otherwise fetch the page with Jimu web/document tools.
...
| Optional upstream MCP form | CLI command alternative |
|----------|-------------|
...
If Context7 is configured in Jimu, resolve the library ID first (one-time per session):
